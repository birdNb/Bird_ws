...
chmod +x ble_gatt_server.py ble_ros_bridge.py ble_neck_bridge.py ble_motor_power_manager.py ble_locate_face_manager.py ble_command_dispatcher.py ble_status_telemetry.py ble_log.py neck_smooth_home.py run_ble_with_ros.sh ros_env.sh 2>/dev/null || true

# ROS2 环境（见 ros_env.sh）
# shellcheck disable=SC1091
source "$(pwd)/ros_env.sh"

if ! python3 -c "import rclpy" 2>/dev/null; then
  echo "[error] 本机无法 import rclpy，BLE 无法转发 ROS2 /joy"
  echo "  请执行: source /opt/ros/foxy/setup.bash"
  echo "  或加 --no-ros 仅做 BLE 打印测试"
  exit 1
fi
if ! python3 -c "from sensor_msgs.msg import Joy" 2>/dev/null; then
  echo "[warn] 无法 import sensor_msgs — 请 source /opt/ros/foxy/setup.bash"
fi
...
echo " 协议: ${PKG_DIR}/docs/BLE_PROTOCOL.md | 参考: ${PKG_DIR}/docs/miniprogram_ble_snippet.js"
