#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""手柄组合键解析：文本 → 标准化组合键（ROS2 桥接到 /joy，非文本转发）。"""
...
# 单键（无 + ）也视为手柄指令
SINGLE_GAMEPAD_KEYS = frozenset(
    {"a", "b", "x", "y", "lb", "rb", "back", "start", "center", "ls", "rs"}
)
...
def combo_to_wire(combo: str) -> str:
    ...
    for p in combo.split("+"):
        if p in ("start", "back", "center", "ls", "rs"):
            out.append(p)
