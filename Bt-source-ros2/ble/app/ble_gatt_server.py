...
class BleGattServer:
    ...
    def _try_report_ready(self) -> None:
        ...
        # ROS2 启动默认 EXEC_DEFAULT=5，不再发 ROS1 的 fsm:2。
        mode_wire = "M_default"
        fsm_val = 5
        if self._ros_bridge is not None:
            try:
                mode_wire = self._ros_bridge.get_mode_wire() or mode_wire
                fsm_val = int(self._ros_bridge.get_fsm_value() or fsm_val)
            except Exception:
                pass
        self._send_plain_notify(f"mode:{mode_wire}")
        self._send_plain_notify(f"fsm:{fsm_val}")
    ...
    def _handle_wifi(self, payload: str) -> None:
        """WIFI 配网：解析 SSID/密码 → nmcli 保存并连接 → FFE2 回最终结果。"""
        from ble_wifi_manager import WIFI_PAYLOAD_SEP, format_wifi_result

        if self._wifi is None:
            log_warn("[wifi] 管理器未就绪")
            self._send_command_echo(format_wifi_result(False, "unavailable"))
            return
        if WIFI_PAYLOAD_SEP not in (payload or ""):
            log_warn("[wifi] 载荷格式错误")
            self._send_command_echo(format_wifi_result(False, "bad_format"))
            return
        ssid, password = payload.split(WIFI_PAYLOAD_SEP, 1)
        ssid = (ssid or "").strip()
    ...
    def _start_wifi_manager(self) -> None:
        try:
            from ble_wifi_manager import WifiManager
        except ImportError as e:
    ...
    def _start_ros_bridge(self) -> None:
        ...
        if self._ros_bridge.start():
            log_info("ROS2 控制桥接已启动（/joy）")
        else:
            log_info("ROS2 桥接后台等待 DDS / 量产算法（BLE 广播先启动）")
    ...
    def _on_ros_mode(self, mode_key: str) -> None:
        if self._telemetry is not None:
            ...
            if self._ros_bridge is not None:
                try:
                    fsm = self._ros_bridge.get_fsm_value()
                except Exception:
                    fsm = None
            try:
    ...
    def _on_gait_state(self, state: str) -> None:
        # AMP running/standby 后回传 GAIT ON/OFF（由 ble_ros_bridge 回调）
        if state in ("ON", "OFF"):
