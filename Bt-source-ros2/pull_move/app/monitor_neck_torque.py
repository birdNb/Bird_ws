...
# -*- coding: utf-8 -*-
"""
脖子 pitch / roll 力矩实时监控。

优先订阅:
  /error_joint_states  (sensor_msgs/JointState.effort, N·m)

默认关节:
  pitch → head_pitch_joint   (上下点头)
  roll  → head_yaw_joint     (左右；机型配置无独立 head_roll，左右轴名为 yaw)

用法:
  ./run_neck_monitor.sh
  ./run_neck_monitor.sh --print-hz 20
  ./run_neck_monitor.sh --plot --save neck_tau.csv
"""
    ...
    def __init__(
        ...
        *,
        ...
        if source == "joint_state":
            ...
            rospy.loginfo("[neck_torque] 订阅 %s (JointState.effort)", joint_state_topic)
            rospy.loginfo(
                "[neck_torque] joints: pitch=%s  roll=%s",
                joints["pitch"],
                joints["roll"],
            )
        else:
            ...
            for label, joint in joints.items():
                ...
                rospy.loginfo("[neck_torque] 订阅 %s (%s)", topic, label)
    ...
    def _on_joint_state(self, msg: JointState) -> None:
        ...
        if any(i is None for i in idxs.values()):
            if not self._warned_missing:
                ...
                rospy.logwarn(
                    "[neck_torque] JointState 缺少: %s；有: %s",
                    missing,
                    ", ".join(msg.name[:24]),
                )
            return
        if not msg.effort or max(idxs.values()) >= len(msg.effort):  # type: ignore[type-var]
            if not self._warned_missing:
                self._warned_missing = True
                rospy.logwarn("[neck_torque] JointState.effort 为空，无法读力矩")
            return
...
def save_csv(
    ...
    rospy.loginfo("[neck_torque] 已保存 CSV: %s (%d 点)", path, len(t))
...
def _diagnose(topic: str) -> None:
    ...
    if topic in pubs:
        rospy.loginfo("[neck_torque] 话题存在: %s  type=%s", topic, pubs[topic])
    else:
        matched = [t for t in pubs if "error_joint" in t or "livelybot_real" in t]
        rospy.logwarn(
            "[neck_torque] 未找到 %s。相关话题: %s",
            topic,
...
def run(args: argparse.Namespace) -> None:
    rospy.init_node("neck_torque_monitor", anonymous=True)
    ...
    if args.source == "auto":
        ...
        if args.joint_state_topic in pubs:
            ...
            rospy.logwarn(
                "[neck_torque] %s 不存在，回退到 MotorState",
    ...
    rospy.loginfo("[neck_torque] 等待力矩数据… (source=%s)", source)
    t_wait = time.time()
    last_print = 0.0
    while not rospy.is_shutdown():
        ...
        if all(latest[a] is not None for a in AXES):
            rospy.loginfo(
                "[neck_torque] 首帧 pitch=%+.3f  roll=%+.3f N*m",
                latest["pitch"],
                latest["roll"],
            )
            break
        ...
        if now - last_print > 2.0:
            last_print = now
            rospy.loginfo(
                "[neck_torque] 仍在等待… pitch=%s roll=%s (已 %.1fs)",
                latest["pitch"],
                latest["roll"],
                now - t_wait,
            )
    ...
    rospy.loginfo("[neck_torque] 开始持续打印 pitch/roll (%.1f Hz)，Ctrl+C 退出", print_hz)
    try:
        while not rospy.is_shutdown():
            latest = monitor.latest()
            if all(latest[a] is not None for a in AXES):
                ...
                print(
                    "t=%7.2fs  pitch=%+.4f  roll=%+.4f  N*m"
                    % (t_rel, latest["pitch"], latest["roll"]),
                    flush=True,
                )
            if stop_at is not None and time.time() >= stop_at:
                rospy.loginfo("[neck_torque] 到达 --duration，结束")
                break
            time.sleep(dt)
    except KeyboardInterrupt:
        rospy.loginfo("[neck_torque] 用户中断")
    ...
    if args.save:
        ...
        if ht:
            ...
            rospy.logwarn("[neck_torque] 无数据可保存到 %s", args.save)
...
def _run_plot(
    ...
    if not os.environ.get("DISPLAY") and matplotlib.get_backend().lower() == "agg":
        rospy.logwarn("[neck_torque] 无 DISPLAY，跳过绘图")
        return
    ...
    colors = {"pitch": "#1f77b4", "roll": "#2ca02c"}
    ...
    def update(_frame):
        ...
        if t:
            for a in AXES:
                ...
                _auto_ylim(axes[axis_idx[a]], series[a])
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="监控脖子 pitch/roll 力矩")
    p.add_argument(
        "--source",
    ...
    p.add_argument(
        "--roll-joint",
        default=ROLL_JOINT,
        help="默认 head_yaw_joint（机型无独立 head_roll）",
    )
