...
def build_adv_payload(name: str = "") -> bytes:
    """主包：Flags + 128-bit FFE0（安卓 services 过滤依赖完整 UUID）。

    主广播仅 31 字节，放不下 Flags + 128-bit UUID + 完整设备名。
    设备名放在 Scan Response，iOS 主动扫描同样能拿到 localName。
    """
    _ = name
    parts = [bytes([0x02, 0x01, 0x06])]
    parts.append(bytes([0x11, 0x07]) + FFE0_UUID128_LE)
    return b"".join(parts)[:31]


def build_scan_rsp(name: str = "") -> bytes:
    """Scan Response：Complete Local Name（iOS / 按名过滤）。"""
    name_b = (name or "HT_BLE").encode("utf-8")[:29]
    if not name_b:
        return b""
    return (bytes([len(name_b) + 1, 0x09]) + name_b)[:31]
