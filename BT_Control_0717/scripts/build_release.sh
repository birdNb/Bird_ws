#!/bin/bash
# 将 BT_Control_0717 源码打成可移植发布包（字节码，不含 .py 源码）
# 输出目录: ../BT_Control_0717_bag
set -euo pipefail

SRC_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_SRC="${SRC_DIR}/app"
VERSION="$(tr -d '[:space:]' < "${SRC_DIR}/VERSION" 2>/dev/null || echo 0717)"
OUT_NAME="BT_Control_${VERSION}_bag"
...
cp "${SRC_DIR}/docs/"* "${OUT_DIR}/docs/" 2>/dev/null || true

# 运行时从 app/ 复制
for f in start.sh run_ble_with_ros.sh ros_env.sh platform_env.sh platform_hw.sh install-autostart.sh; do
  cp "${APP_SRC}/${f}" "${APP_DIR}/"
done
cp "${APP_SRC}/systemd/"* "${APP_DIR}/systemd/"
cp "${APP_SRC}/scripts/check.sh" "${APP_SRC}/scripts/recover.sh" "${APP_DIR}/scripts/"

for py in "${PY_FILES[@]}"; do
  if [ ! -f "${APP_SRC}/${py}" ]; then
    echo "[warn] 跳过缺失: ${py}"
    continue
  fi
  cp "${APP_SRC}/${py}" "${APP_DIR}/"
done
...
# Bird BLE 遥控安装包 (BT_Control_${VERSION}_bag)
