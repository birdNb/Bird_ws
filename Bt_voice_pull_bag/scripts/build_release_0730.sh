...
# 包根蓝牙名（默认出厂名）
echo "HT_88888888" >"${OUT_DIR}/ble_device_name.conf"
chmod 664 "${OUT_DIR}/ble_device_name.conf"
rm -f "${OUT_DIR}/ble/ble_device_name.conf"

# platform_env：优先读包根 conf
if [ -f "${OUT_DIR}/ble/app/platform_env.sh" ]; then
  cat >"${OUT_DIR}/ble/app/platform_env.sh" <<'PEOF'
#!/bin/bash
